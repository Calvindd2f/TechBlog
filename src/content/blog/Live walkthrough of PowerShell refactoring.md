---
title: Live walkthrough of PowerShell refactoring
description: Live walkthrough of how I refactor PowerShell with a real Pull Request.
pubDate: Sept 6 2024
heroImage: /blog-placeholder-1.jpg
tags: ['Modularity', 'Performance', 'Refactoring']
categories: ['Refactoring']
---

So I had stumbled upon this aboslute dumpsterfire of a repository with a Windows 11 ISO creation utility. I haven't been in the custom created ISO game in quite some time but when I was I thoroughly enjoyed just using NTLite.

[Tiny11](https://github.com/ntdevlabs/tiny11builder/blob/main/tiny11Coremaker.ps1)

I will dump the entirety of the core script file here as it's quite something... It will assist in the before and after; and yes - I did say script file. It is not a module, there are 0 functions in the script. It doesn't even start with a param block.

On restrospect, I will go over each step one-by-one showcasing the orignal and then the changes.

The full original will be at the bottom alongside the refactored edit beneath it
...
Wrapped into function with a `skip` switch. Added `begin/process/end`. Added `try/catch` for error handling, the first `catch` attempts another `try/catch` by setting execution policy to `Bypass` for current process only. If that fails, it prints the error information. Each has a `$success` variable which if the end block evaluation is `true` it succeeds, if it is `false` it allows the user to read the errors then only exit on input.
...
function EP_chk {
   ...
   {
      if ((Get-ExecutionPolicy) -eq 'Restricted') {
         [console]::WriteLine("Your current PowerShell Execution Policy is set to Restricted, which prevents scripts from running. Changing to Remote Signed.")
      }
   }
   process
   {
      ...
      {
         Set-ExecutionPolicy RemoteSigned -Scope CurrentUser -Confirm:$false
         [console]::WriteLine('ExecutionPolicy Changed.')
         $success=$true;
      }
      catch [System.Exception]
      {
         ...
         catch
         {
            ...
            $EH = [PSCustomObject]@{
               ...
               RecommendedAction = $PSItem.ErrorDetails.RecommendedAction
            }
            foreach ($e in $eh) { [console]::WriteLine($e); }
   ...
   end
   {
      ...
      elseif ($success -eq $false){
         [console]::WriteLine('The script cannot be run without changing the execution policy. Exiting...')
         $x = Read-Host -message 'exit'
         if ($x.Length -gt 0) { exit[1] }
      }
   }
}
```
...
+ Replaced `Write-Host` with `[console]::WriteLine(' ').
+ Wrapped a lot of loose snippets into functions.
+ Implemented advanced error handling for each function.
+ Opted for heavy usage of .NET method calls as the script also contains inline C#, so their is little reason to argue over idiomatic powershell and just goes for raw performance gains.
